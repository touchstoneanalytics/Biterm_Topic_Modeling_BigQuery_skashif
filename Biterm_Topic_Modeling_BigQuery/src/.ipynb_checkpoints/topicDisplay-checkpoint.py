...
def dispTopics(pt, voca, pz):
    ...
    for l in open(pt):
        ...
        tmps = ' '.join(['%s:%f' % (voca[w],v) for w,v in wvs[:10]]) # changed here for getting top 5 words in a topic
...
if __name__ == '__main__':
    ...
    for pz, s in sorted(topics, reverse=True):
        ...
        topics_dict[index] = dict([i for i in kv.split(':')] for kv in s.split())
        index += 1
    
    with open('/mnt/output/topics.pickle', 'wb') as handle:
        pickle.dump(topics_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)