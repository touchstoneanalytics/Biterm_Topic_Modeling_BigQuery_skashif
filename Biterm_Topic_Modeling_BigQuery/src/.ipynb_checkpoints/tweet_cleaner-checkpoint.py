...
class Data_Cleaner():
    ...
    def clean_data(self):
        ...
        stop = stopwords.words(['english', 'french', 'german', 'russian', 'spanish', 'dutch'])
        # adding custom words from our datasets that we want to ignore
        #stop.extend(['cnet', '-', '|', 'says', '@applesupport', '@amazonhelp', 'applesupport', 'amazonhelp', 'das', 'ich', 'und', 'la', 'el', 'ist', 'un', 'auf', 'se', 'en', 'que', 'nicht', 'est', 'meu','depuis', 'não', 'si','ça', 'bu', 'bir', 'bi'])
        
        stop.extend(['cnet', '-', '|', 'says', '@applesupport', '@amazonhelp', 'applesupport', 'amazonhelp', 'go', 'get'])

        # reading the tweets file
        tweets = pd.read_csv(self.file_path, usecols = ['text'], encoding='utf-8')
        ...
        tweets['text_clean'] = tweets['text'].apply(lambda x: ' '.join([lemma.lemmatize(word) for word in x.split() 
                                                                                                if word not in (stop)]))
